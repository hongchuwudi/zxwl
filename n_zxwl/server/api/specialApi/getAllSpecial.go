...
func ProfessionalQueryHandlers(w http.ResponseWriter, r *http.Request) {
	...
	response := oldModel.ProfessionalQueryResponse{Code: 1001}
	...
	// 解析请求参数
	var req oldModel.ProfessionalQueryRequest
	fmt.Printf("%+v\n", req)
	...
	// 初始化repository
	specialRepo := specialRepo.NewSpecialRepository(config.GetDB())
	...
	// 转换为响应格式
	var results []oldModel.ProfessionalItemResponse
	for _, special := range specials {
		...
		item := oldModel.ProfessionalItemResponse{
			ID:             special.ID,
			Name:           special.Name,
