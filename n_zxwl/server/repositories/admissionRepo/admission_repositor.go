...
func (r *AdmissionRepository) GetTypeIDByName(name string) (int, error) {
	var admissionType sqlModel2.AdmissionType
	...
	// 如果精确匹配没找到，尝试模糊匹配
	var admissionTypes []sqlModel2.AdmissionType
...
func (r *AdmissionRepository) GetTypeByName(name string) (*sqlModel2.AdmissionType, error) {
	var admissionType sqlModel2.AdmissionType
...
func (r *AdmissionRepository) GetAllTypes() ([]sqlModel2.AdmissionType, error) {
	var types []sqlModel2.AdmissionType
	err := r.db.Find(&types).Error
	return types, err
}

// FilterAdmissionData 调用存储过程筛选录取数据
func (r *AdmissionRepository) FilterAdmissionData(input sqlModel2.UserInput) ([]map[string]interface{}, error) {
