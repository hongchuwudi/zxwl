...
import (
	...
	myRepo "mymod/new_zxwl/repositories"
	"regexp"
	"strconv"
	"strings"
)
...
type RecommendationRepository struct {
	db                       *gorm.DB
	admissionRepo            *myRepo.AdmissionRepository
	universityRepo           *myRepo.UniversityRepository
	specialRepo              *myRepo.SpecialRepository
	recommendationRepo       *myRepo.RecommendationRepo
...
func NewRecommendationRepository(db *gorm.DB) *RecommendationRepository {
	...
	return &RecommendationRepository{
		db:                       db,
		admissionRepo:            myRepo.NewAdmissionRepository(db),
		universityRepo:           myRepo.NewUniversityRepository(db),
		specialRepo:              myRepo.NewSpecialRepository(db),
		recommendationRepo:       myRepo.NewRecommendationRepo(db),
...
func (r *RecommendationRepository) GetRecommendations(priInput sqlModel.UserPriInput) (sqlModel.UserInput, sqlModel.Recommendation, error) {
	...
	if priInput.TypeName != "" {
		typeID, err := myRepo.NewAdmissionRepository(r.db).GetTypeIDByName(priInput.TypeName)
		if err != nil {
	...
	var err1, err2 error
	// 先查生源省份ID,这个是必查的
	input.ProvinceID, err2 = myRepo.NewProvinceRepository(r.db).GetProvinceIDByName(priInput.ProvinceName)
	if priInput.GoalProvinceName == "" {
		input.GoalProvinceID = 0
	} else {
		input.GoalProvinceID, err1 = myRepo.NewProvinceRepository(r.db).GetProvinceIDByName(priInput.GoalProvinceName)
	}
	if err1 != nil {
	...
	// 调用存储过程
	equivalentResult, err := myRepo.NewScoreRepository(r.db).GetEquivalentScore(
