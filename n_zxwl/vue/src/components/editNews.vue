    ...
    <div class="drawer-content">
      ...
      >
        ...
        <el-form-item label="是否推送" prop="is_push">
          <el-radio-group v-model="formData.is_push">
...
const drawerTitle = computed(() => {
  return formData.title ? `编辑: ${formData.title}` : '编辑资讯文章'
})
...
const fetchNewsDetail = async () => {
  ...
  try {
    ...
    const response = await axios.get(`/gapi/news/detail?id=${props.newsId}`)
    ...
    if (response.data.error === 0 && response.data.data) {
      ...
      Object.assign(formData, {
        ...
        province_id: news.province_id || '',
        style_url: news.style_url || '',
        description: news.description || '',
        content: news.content || '',
        class_name: news.class_name || '',
        is_push: news.is_push || 2,
        is_top: news.is_top || 1,
        from_source: news.from_source || '',
        publisher_email: news.publisher_email || ''
      })
    } else {
...
const handleClose = () => {
  if (isFormDirty()) {
    ElMessageBox.confirm('确定要取消编辑吗？已修改的内容将不会保存', '提示', {
...
// 检查表单是否被修改
const isFormDirty = () => {
  return formData.title || formData.content || formData.keywords
}
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const submitData = {
      ...
      style_url: formData.style_url,
      description: formData.description,
      content: formData.content,
      class_name: formData.class_name,
      is_push: formData.is_push,
      is_top: formData.is_top,
      from_source: formData.from_source || userName.value,
      publisher_email: formData.publisher_email || userEmail.value
    }

    // 调用更新API
    const response = await axios.put('/gapi/news/updateAll', submitData)
